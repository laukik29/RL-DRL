...
def gaussian_likelihood(x, mu, log_std):
    ...
    #                     #
    #######################
    EPS = 1e-8 #small numerical const to avoid instability
    #compute the pre_sum term in the log likelihood expression
    pre_sum = -0.5*(((x-mu)/(tf.exp(log_std) + EPS))**2 + 2*log_std + np.log(2*np.pi))
     # sum along the second axis (num_dimensions) to get log likelihood of each sample 
